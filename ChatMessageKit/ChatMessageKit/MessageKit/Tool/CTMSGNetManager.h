...
//
+ (void)getConversationDetail:(NSString *)targetId lastId:(nullable NSString *)lastId success:(void (^)(id response))success failure:(void (^)(NSError *err))failure;
...
+ (void)sendMessage:(NSDictionary *)dic success:(void (^)(id response))success failure:(void (^)(NSError *err))failure NS_UNAVAILABLE;
