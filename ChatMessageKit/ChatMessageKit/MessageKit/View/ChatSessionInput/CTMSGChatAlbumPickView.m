...
//        self.albumModel.selectedCount--;
    ...
    for (int i = 0; i < self.albumManager.selectedList.count; i++) {
        ...
        [cell.pickBtn setTitle:[NSString stringWithFormat:@"%d", (int)(model.selectedIndex + 1)] forState:UIControlStateNormal];
    }
...
- (void)p_getAlbumPhotos {
    __weak typeof(self) weakSelf = self;
    [self.albumManager fetchPhotoForPHResult:self.allPhotoModel.result limitCount:50 index:0 result:^(NSArray<CTMSGPhotoModel *> * _Nonnull photos, NSArray<CTMSGPhotoModel *> * _Nonnull videos, NSArray<CTMSGPhotoModel *> * _Nonnull objs) {
...
- (void)p_commonInit {
    ...
    _albumManager = [[CTMSGAlbumManager alloc] init];
