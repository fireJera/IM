...
+ (NSString *)maximumOfJudgment:(CTMSGPhotoModel *)model viewmodel:(CTMSGAlbumManager *)viewmodel {
    ...
    if (viewmodel.selectionType == AlbumListSelectionTypeAll) {
        if ((model.type == CTMSGPhotoModelMediaTypePhoto || model.type == CTMSGPhotoModelMediaTypeGif) ||
            ...
            //            else if (model.type == CTMSGPhotoModelMediaTypeVideo || model.type == PhotoModelMediaTypeCameraVideo) {
...
//+ (void)getImageForSelectedPhoto:(NSArray<CTMSGPhotoModel *> *)photos type:(HXPhotoToolsFetchType)type completion:(void (^)(NSArray<UIImage *> *))completion {
//    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
