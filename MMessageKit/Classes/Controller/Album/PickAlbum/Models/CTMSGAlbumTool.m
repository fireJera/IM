...
@implementation CTMSGAlbumTool

//+ (UIImage *)ctmsg_imageNamed:(NSString *)imageName {
//    UIImage *image = [UIImage imageNamed:imageName];
//    if (image) {
//        return image;
//    }
//    NSString *path = [NSString stringWithFormat:@"CTMSGWeiboPhotoPicker.bundle/%@", imageName];
//    image = [UIImage imageNamed:path];
//    if (image) {
//        return image;
//    } else {
//        NSString *path = [NSString stringWithFormat:@"Frameworks/CTMSGWeiboPhotoPicker.framework/CTMSGWeiboPhotoPicker.bundle/%@",imageName];
//        image = [UIImage imageNamed:path];
//        if (!image) {
...
+ (UIImage *)ctmsg_imageNamed:(NSString *)imageName {
    ...
    NSString * path = [selfBundle pathForResource:@"ChatKitPhotoPicker" ofType:@"bundle"];
...
//+ (void)isICloudAssetWithModel:(CTMSGPhotoModel *)model
...
//}
//
//+ (PHImageRequestID)getImageWithAlbumModel:(CTMSGAlbumModel *)model
//                                      size:(CGSize)size
//                                completion:(void(^)(UIImage *image, CTMSGAlbumModel *model))completion {
//    PHImageRequestOptions *option = [[PHImageRequestOptions alloc] init];
//    option.resizeMode = PHImageRequestOptionsResizeModeFast;
...
+ (PHImageRequestID)getImageWithModel:(CTMSGPhotoModel *)model
                           completion:(void(^)(UIImage *image, CTMSGPhotoModel *model))completion {
    PHImageRequestOptions *option = [[PHImageRequestOptions alloc] init];
    option.resizeMode = PHImageRequestOptionsResizeModeFast;
...
//+ (void)getFullSizeImageUrlFor:(CTMSGPhotoModel *)model
              ...
              completion:(void (^)(NSString *))completion
{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        for (int i = 0 ; i < photos.count ; i++) {
            CTMSGPhotoModel *model = photos[i];
...
+ (NSString *)maximumOfJudgment:(CTMSGPhotoModel *)model viewmodel:(CTMSGAlbumManager *)viewmodel {
    if (viewmodel.selectedList.count == viewmodel.maxNum) {
        return [NSString stringWithFormat:@"最多只能选择%ld个", (long)viewmodel.maxNum];
    }
//    if (viewmodel.selectionType == CTMSGAlbumListSelectionTypeAll) {
...
//                // 已经达到图片最大选择数
//                return [NSString stringWithFormat:@"最多只能选择%ld张图片", (long)viewmodel.maxPhotoNum];
//            }
//        }
...
//                // 已经达到视频最大选择数
//                return [NSString stringWithFormat:@"最多只能选择%ld个视频", (long)viewmodel.maxVideoNum];
//            }
//        }
//    }
//    else if (viewmodel.selectionType == CTMSGAlbumListSelectionTypePhoto) {
//        if (viewmodel.selectedPhotos.count == viewmodel.maxPhotoNum) {
//            // 已经达到图片最大选择数
//            return [NSString stringWithFormat:@"最多只能选择%ld张图片", (long)viewmodel.maxPhotoNum];
//        }
//    }
//    else if (viewmodel.selectionType == CTMSGAlbumListSelectionTypeVideo) {
//        if (viewmodel.selectedVideos.count == viewmodel.maxVideoNum) {
//            // 已经达到视频最大选择数
//            return [NSString stringWithFormat:@"最多只能选择%ld个视频",(long)viewmodel.maxVideoNum];
//        }
...
//            return [NSString stringWithFormat:@"视频少于%ld秒,无法选择", (long)viewmodel.minVideoDuration];
//        }else if (model.asset.duration > viewmodel.maxVideoDuration) {
//            return [NSString stringWithFormat:@"视频大于%ld,无法选择", (long)viewmodel.minVideoDuration];
//        }
//    }
    return nil;
}
...
//+ (void)getImageForSelectedPhoto:(NSArray<CTMSGPhotoModel *> *)photos type:(CTMSGAlbumToolFetchType)type completion:(void (^)(NSArray<UIImage *> *))completion {
//    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
//        NSMutableArray *HDImages = [NSMutableArray array];
//        __weak typeof(self) weakSelf = self;
//        for (CTMSGPhotoModel *model in photos) {
...
//                    });
//                }];
//            } else if (model.type == CTMSGPhotoModelMediaTypePhotoGif) {
...
//                    }];
//                }];
//            }/*else if (model.type == CTMSGPhotoModelMediaTypeLivePhoto) {
...
//                  }else {
//                      CGSize size;
//                      if (type == CTMSGAlbumToolFetchHDImageType) {
...
//+ (void)sortImageForModel:(CTMSGPhotoModel *)model total:(NSInteger)total images:(NSMutableArray *)images completion:(void(^)(NSArray *array))completion {
//    [images addObject:model];
//    if (images.count == total) {
//        [images sortUsingComparator:^NSComparisonResult(CTMSGPhotoModel *temp, CTMSGPhotoModel *other) {
...
//        for (CTMSGPhotoModel *md in images) {
//            if (!md.previewPhoto) {
...
//}
//
//+ (void)getSelectedPhotosFullSizeImageUrl:(NSArray<CTMSGPhotoModel *> *)photos complete:(void (^)(NSArray<NSURL *> *))complete {
//    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
...
//}
//
//+ (void)sortFullImageUrlForModel:(CTMSGPhotoModel *)model total:(NSInteger)total images:(NSMutableArray *)images completion:(void(^)(NSArray *array))completion {
//    [images addObject:model];
//    if (images.count == total) {
//        [images sortUsingComparator:^NSComparisonResult(CTMSGPhotoModel *temp, CTMSGPhotoModel *other) {
//            NSInteger length1 = temp.endIndex;
...
//+ (void)getSelectedListResultModel:(NSArray<CTMSGPhotoModel *> *)selectedList complete:(void (^)(NSArray<PhotoResultModel *> *, NSArray<PhotoResultModel *> *, NSArray<PhotoResultModel *> *))complete {
//    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
//        NSMutableArray *models = [NSMutableArray array];
//        NSMutableArray *photos = [NSMutableArray array];
//        NSMutableArray *videos = [NSMutableArray array];
//        for (CTMSGPhotoModel *photoModel in selectedList) {
//            PhotoResultModel *resultModel = [[PhotoResultModel alloc] init];
//            resultModel.index = photoModel.endCollectionIndex;
//            if (photoModel.asset) {
//                if (photoModel.subType == CTMSGPhotoModelMediaSubTypeVideo) {
...
//                if (photoModel.subType == CTMSGPhotoModelMediaSubTypePhoto) {
//                    resultModel.photoIndex = photoModel.endIndex;
//                    resultModel.type = PhotoResultModelMediaTypePhoto;
...
//        for (CTMSGPhotoModel *photoModel in selectList) {
//            if (photoModel.subType == CTMSGPhotoModelMediaSubTypePhoto) {
//                NSString *suffix;
//                if (photoModel.asset) {
//                    if (photoModel.type == CTMSGPhotoModelMediaTypePhotoGif) {
//                        suffix = @"gif";
...
//+ (void)writeOriginalImageToTempWith:(CTMSGPhotoModel *)model
...
//}
//
//+ (AVAssetExportSession *)compressedVideoWithMediumQualityWriteToTemp:(CTMSGPhotoModel *)model
