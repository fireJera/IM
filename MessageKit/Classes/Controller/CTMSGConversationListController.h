//
//  CTMSGConversationListController.h
//  ChatMessageKit
//
...
@class CTMSGConversationModel, CTMSGConversationBaseCell, CTMSGNetworkIndicatorView;
NS_ASSUME_NONNULL_BEGIN

@interface CTMSGConversationListController : UIViewController <UITableViewDelegate, UITableViewDataSource>
...
/*!
 ...
 @param displayConversationTypeArray    列表中需要显示的会话类型数组(需要将CTMSGConversationType转为NSNumber构建Array)
 @param collectionConversationTypeArray
 列表中需要聚合为一条显示的会话类型数组(需要将CTMSGConversationType转为NSNumber构建Array)
 @return                                    会话列表对象
...
- (instancetype)initWithDisplayConversationTypes:(NSArray *)displayConversationTypeArray
                      collectionConversationType:(NSArray *)collectionConversationTypeArray NS_UNAVAILABLE;
...
/*!
 ...
 @discussion 数组中的元素为CTMSGConversationType转换的NSNumber
 */
...
/*!
 ...
 @discussion 数组中的元素为CTMSGConversationType转换的NSNumber
 */
@property(nonatomic, strong) NSArray *collectionConversationTypeArray NS_UNAVAILABLE;
 ...
 */
@property(nonatomic, assign) BOOL isEnteredToCollectionViewController NS_UNAVAILABLE;

#pragma mark - 列表属性
/*!
 列表中会话数据模型的数据源
 
 @discussion 数据源中存放的元素为会话Cell的数据模型，即CTMSGConversationModel对象。
 @warning 非线程安全，请在主线程操作此属性
 */
@property(nonatomic, strong) NSMutableArray<CTMSGConversationModel *> *conversationListDataSource;
...
/*!
 ...
 请在viewDidLoad之前设置，此设置在SDK中全局有效。
 */
- (void)setConversationPortraitSize:(CGSize)size NS_UNAVAILABLE;
 ...
 */
- (void)onSelectedTableRow:(CTMSGConversationModelType)conversationModelType
         conversationModel:(CTMSGConversationModel *)model
...
/*!
 ...
 @param model   会话Cell的数据模型
 */
- (void)didLongPressCellPortrait:(CTMSGConversationModel *)model NS_UNAVAILABLE;
...
/*!
 ...
 @param dataSource      即将加载的列表数据源（元素为CTMSGConversationModel对象）
 @return                修改后的数据源（元素为CTMSGConversationModel对象）
 
 @discussion 您可以在回调中修改、添加、删除数据源的元素来定制显示的内容，会话列表会根据您返回的修改后的数据源进行显示。
 数据源中存放的元素为会话Cell的数据模型，即CTMSGConversationModel对象。
 */
 ...
 */
- (CTMSGConversationBaseCell *)CTMSGConversationListTableView:(UITableView *)tableView
...
/*!
 ...
 如果确定删除该会话，您需要在调用CTMSGIMClient中的接口删除会话或其中的消息，
 并从conversationListDataSource和conversationListTableView中删除该会话。
 */
- (void)CTMSGConversationListTableView:(UITableView *)tableView
...
/*!
 当用户退出登录时，是否还能继续显示会话列表
 
 @discussion 默认值为YES。
 */
@property(nonatomic, assign) BOOL showConversationListWhileLogOut NS_UNAVAILABLE;
...
/*!
 ...
 notification的object为CTMSGMessage消息对象，userInfo为NSDictionary对象，其中key值为@"left"，value为还剩余未接收的消息数的NSNumber对象。
 */
 ...
 */
@property(nonatomic, strong) CTMSGNetworkIndicatorView *networkIndicatorView;
